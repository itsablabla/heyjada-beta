...
describe('buildConfirmationSummary', () => {
    ...
    test('shell commands read as one intent sentence, with risk and file count as follow-ons', () => {
        ...
        expect(out).toContain('Jada wants to run a read-write shell command to clean the build directory.');
    ...
    test('edits name the file in spoken form', () => {
        ...
        expect(buildConfirmationSummary(r)).toContain('Jada wants to edit Tasks.org under the Documents/Notes folder.');
    });
    ...
    test('mcp tool calls speak the humanized tool name', () => {
        ...
        expect(buildConfirmationSummary(r)).toContain('Jada wants to use the github create issue tool.');
    });
