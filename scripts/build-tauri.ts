...
const SKIP_DIRECTORIES = new Set([
    ...
    ".git",
    "dist",
    "platform", // Superjoy Platform is a separate service
]);
...
async function buildTauri(debug: boolean, platform: Platform, disableUpdaterArtifacts: boolean) {
    ...
    if (platform.startsWith("darwin") && debug) {
        const appPath = path.join(ROOT_DIR, "src-tauri", "target", "debug", "bundle", "macos", "Superjoy.app");
...
async function main() {
    ...
    console.log("🍞 Superjoy Tauri Desktop Build (Bundled Runtimes)");
