 ...
 * variables (light + dark) to feel like a trusted, native part of Super Joy.
 *
...
export interface StatusPageOptions {
    ...
    /** When set, shows an "Open Super Joy" button and (on success) auto-opens it. */
    deepLink?: string;
...
export function renderStatusPage(opts: StatusPageOptions): string {
    ...
    const button = deepLink
        ? `<a href="${escapeHtml(deepLink)}" class="btn" id="open-app">Open Super Joy</a>`
        : '';
...
<head>
    <meta charset="utf-8">
    <title>Super Joy - ${safeTitle}</title>
