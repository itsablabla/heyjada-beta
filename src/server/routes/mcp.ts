...
mcp.get('/oauth/callback', async (c) => {
    ...
    if (!server) {
        return renderOAuthCallbackPage('MCP server not found', 'Super Joy could not find the MCP server for this OAuth callback.', false);
    }
    ...
    try {
        ...
        return renderOAuthCallbackPage(`${server.name} connected`, 'Authorization complete. Returning you to Super Joy — you can close this tab once the app is in focus.', true);
    } catch (err) {
