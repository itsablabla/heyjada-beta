...
// Conversation Folder Schema — user-defined folders (and sub-folders) to organize chats
export const ConversationFolder = pgTable('conversation_folder', {
    id: uuid('id').defaultRandom().notNull().primaryKey(),
    userId: integer('user_id').notNull().references(() => User.id, { onDelete: 'cascade' }),
    name: text('name').notNull(),
    // Optional parent folder - if set, this folder is a sub-folder
...
export const Conversation = pgTable('conversation', {
    ...
    // Optional folder for organizing chats - conversations without a folder live at the root
