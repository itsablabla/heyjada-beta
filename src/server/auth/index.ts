 ...
 *
 * Handles authentication with the Superjoy Platform for the local app.
 ...
 * This sets up Superjoy Platform as an AI provider and adds available models
 */
export async function syncPlatformModels(): Promise<void> {
    ...
    try {
        ...
        // Check if Superjoy provider already exists
        ...
        if (existingProvider) {
            ...
            if (existingProvider.apiBaseUrl !== expectedBaseUrl) {
                log.info({ oldUrl: existingProvider.apiBaseUrl, newUrl: expectedBaseUrl }, 'Updating Superjoy provider base URL');
                await db
                    .update(AiModelApi)
                    .set({ apiBaseUrl: expectedBaseUrl, updatedAt: new Date() })
                    .where(eq(AiModelApi.id, providerId));
            }
            log.debug('Superjoy provider already exists');
        } else {
            // Create the Superjoy provider
            ...
            if (!newProvider) {
                log.error('Failed to create Superjoy provider');
                return;
            }

            providerId = newProvider.id;
            log.info('Created Superjoy provider');
        }
 ...
 * This sets up Superjoy as a web search and web scraper provider
...
export async function syncPlatformWebTools(): Promise<void> {
    ...
    try {
        ...
        if (existingSearchProvider) {
            ...
            log.debug('Updated Superjoy web search provider');
        } else {
            // Create the Superjoy web search provider
            ...
            log.info('Created Superjoy web search provider');
        }
        ...
        if (existingScraper) {
            ...
            log.debug('Updated Superjoy web scraper');
        } else {
            // Create the Superjoy web scraper
            ...
            log.info('Created Superjoy web scraper');
        }
